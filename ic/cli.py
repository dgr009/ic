...
from oci_module.search import policy_search as oci_policy_search
...
def main():
    ...
    # ---------------- OCI ----------------
    oci_info_parser = oci_subparsers.add_parser("info", help="OCI 인스턴스/LB/NSG/볼륨/비용 정보 조회")
    oci_info.add_arguments(oci_info_parser)
    oci_info_parser.set_defaults(func=oci_info.main)
    oci_search_parser = oci_subparsers.add_parser("search", help="OCI 검색 관련 명령어")
    oci_policy_search.add_arguments(oci_search_parser)
    oci_search_parser.set_defaults(func=oci_policy_search.main)
