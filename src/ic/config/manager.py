...
class ConfigManager:
    ...
    def __init__(self, security_manager: Optional[SecurityManager] = None):
        ...
        if self.security_manager and not hasattr(self.security_manager, 'config_manager'):
            self.security_manager.config_manager = self
        ...
        for config_path in config_paths:
            ...
            if config_path.exists():
                try:
                    ...
                    sec_file = config_path.parent / "secrets.yaml"
                    if sec_file.exists() and sec_file != config_path:
                        sec_config = self._load_config_file(sec_file)
                        file_config = self._merge_configs(file_config, sec_config)
        ...
        if self.security_manager:
            ...
            if security_warnings:
                for warning in security_warnings:
                    logger.warning(f"Found sensitive data in configuration: {warning}")
    ...
    def validate_config(self, config_data: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        cloud_sections = ['aws', 'gcp', 'oci', 'cloudflare']
        if not any(section in config_data for section in cloud_sections):
            errors.append("Configuration should include at least one cloud provider section (aws, gcp, oci, cloudflare)")
    ...
    def cleanup_old_backups(self, max_backups: int = 10) -> None:
        ...
        try:
            backup_files = list(set(self._backup_dir.glob("*.yaml")).union(self._backup_dir.glob("*.yml")))
