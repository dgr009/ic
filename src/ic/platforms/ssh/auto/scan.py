...
class SshAutoScanCommand(BaseCommand):
    """SSH Auto Scan & Discovery command."""
    ...
    def execute(self, args, config=None) -> CommandResult:
        ...
        cidr = getattr(args, 'cidr', None) or select_cidr()
        if not cidr:
            console.print("[red]No CIDR specified or detected.[/red]")
            return CommandResult(data=[], success=False, error="No CIDR specified")
...
def main(args, config=None):
    SshAutoScanCommand().run(args, config)


def add_arguments(parser):
    SshAutoScanCommand.add_arguments(parser)
