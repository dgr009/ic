...
try:
    ...
    from ic.platforms.ssh.server_info import (
        parse_ssh_config, collect_all_server_info, display_server_info,
        SSH_CONFIG_FILE, SSH_TIMEOUT, MAX_WORKER, SSH_SKIP_PREFIXES
    )
...
class SshServerInfoCommand(BaseCommand):
    """SSH Server Information command."""
    ...
    def execute(self, args, config=None) -> CommandResult:
        ...
        if not server_configs:
            ...
            return CommandResult(data=[], success=False, error="No SSH configs found")

        if hasattr(args, 'host') and args.host and args.host != 'default':
            original_count = len(server_configs)
            server_configs = [
                c for c in server_configs 
                if args.host.lower() in c['servername'].lower()
            ]
            if not server_configs:
                console.print(f"[red]No servers found matching pattern '{args.host}'.[/red]")
                return CommandResult(data=[], success=True)
            console.print(f"[cyan]Host filter applied:[/cyan] '{args.host}' → {len(server_configs)}/{original_count} servers selected")

        headers = [
            "Server Name", "Access IP", "Internal IP",
            "/ Tot", "/ %", "/app Tot", "/app %", "/data Tot", "/data %",
            "vCPU", "CPU %", "Memory", "Mem %"
        ]
        ...
        try:
            ...
            failed_count = sum(1 for result in results if len(result) > 2 and result[2] == "Connection Fail")
            ...
            for row in results:
                if len(row) == len(headers):
                    ...
                    structured.append({"raw": row})

            return CommandResult(
                data=structured,
                table_renderer=lambda data, verbose=False: display_server_info(results, headers),
            )
        except Exception as e:
            logger.exception(f"Error during server information collection: {e}")
            console.print(f"[bold red]Error:[/bold red] Unexpected error occurred: {e}")
            return CommandResult(data=[], success=False, error=str(e))


def main(args, config=None):
    SshServerInfoCommand().run(args, config)


def add_arguments(parser):
    SshServerInfoCommand.add_arguments(parser)
