...
class TencentClbTrafficCommand(BaseCommand):
    """Tencent Cloud CLB Network Traffic / Bandwidth Statistics command."""
    ...
    def execute(self, args, config=None) -> CommandResult:
        ...
        return CommandResult(
            data=all_rows,
            table_renderer=lambda data, v=False: print_traffic_table(data, days, verbose=v or verbose),
        )


def main(args, config=None) -> None:
    TencentClbTrafficCommand().run(args, config)


def add_arguments(parser) -> None:
    TencentClbTrafficCommand.add_arguments(parser)
