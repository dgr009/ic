...
class GcpBillingInfoCommand(BaseCommand):
    ...
    def execute(self, args, config=None) -> CommandResult:
        ...
        def _table_render(data, verbose=False):
            format_table_output(data, date_range)

        def _tree_render(data):
            format_tree_output(data, date_range)

        if getattr(args, 'mock', False):
            billing_info = load_mock_data()
            filtered = []
            ba_filter = getattr(args, 'billing_account', None)
            proj_filter = getattr(args, 'project', None)
            for b in billing_info:
                if ba_filter and ba_filter.lower() not in str(b.get('billing_account_name', '')).lower():
                    continue
        ...
        if not GCP_BILLING_AVAILABLE:
            console.print("[red]❌ google-cloud-billing 패키지가 설치되지 않았습니다.[/red]")
            console.print("[yellow]   pip install 'ic-cli[gcp]' 또는 pip install google-cloud-billing[/yellow]")
            console.print("[yellow]   Mock 데이터로 테스트하려면 --mock 옵션을 사용하세요.[/yellow]")
            return CommandResult(data=[], error="google-cloud-billing is not installed", success=False)
        ...
        try:
            ...
            filters = {}
            if getattr(args, 'billing_account', None):
...
def main(args, config=None) -> None:
    GcpBillingInfoCommand().run(args, config)


def add_arguments(parser) -> None:
    GcpBillingInfoCommand.add_arguments(parser)
