...
def fetch_pipeline_deploy_stages(account_id, profile_name, region_name, pipeline_name):
    """CodePipeline의 배포 스테이지 정보를 수집합니다."""
    log_info_non_console(f"CodePipeline 배포 스테이지 정보 수집: Account={account_id}, Region={region_name}, Pipeline={pipeline_name}")
    ...
    try:
        ...
        # 배포 관련 스테이지 필터링 (대소문자 구분 없음)
        deploy_stages = []
        for stage in stage_states:
            stage_name = stage.get('stageName', '').lower()
            if 'deploy' in stage_name or 'deployment' in stage_name:
        ...
        if not deploy_stages:
            log_error(f"Error: No stage matching 'deploy' found in pipeline '{pipeline_name}'.")
            return []
...
def format_deploy_stages_table(stages_info):
    """배포 스테이지를 테이블 형식으로 출력합니다."""
    if not stages_info:
        console.print("[yellow]표시할 배포 스테이지가 없습니다.[/yellow]")
        return
    
    console.print(f"\n[bold blue]═══ CodePipeline Deploy Stages ═══[/bold blue]")
...
def collect_codepipeline_deploy_data(args):
    """CodePipeline 배포 스테이지 데이터를 수집합니다."""
    ...
    with ManualProgress(f"Collecting CodePipeline deploy stages for '{args.pipeline_name}' across accounts and regions", total=total_operations) as progress:
        with ThreadPoolExecutor() as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    progress.update(f"Processed {acct}/{reg} - Found {stage_count} deploy stages", advance=1)
                except Exception as e:
                    log_info_non_console(f"Failed to collect CodePipeline deploy data for {acct}/{reg}: {e}")
...
class AwsCodePipelineDeployCommand(BaseCommand):
    """AWS CodePipeline deploy stages command."""
    ...
    def execute(self, args, config=None) -> CommandResult:
        all_stages = collect_codepipeline_deploy_data(args)
        return CommandResult(
            data=all_stages,
            table_renderer=lambda data, verbose=False: format_deploy_stages_table(data),
        )


def main(args, config=None):
    AwsCodePipelineDeployCommand().run(args, config)


def add_arguments(parser):
    AwsCodePipelineDeployCommand.add_arguments(parser)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CodePipeline 배포 스테이지 상태 조회")
