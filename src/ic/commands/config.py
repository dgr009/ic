...
class ConfigCommands:
    ...
    def add_subparsers(self, parent_parser: Any) -> None:
        ...
        init_parser.add_argument(
            "--template", "-t",
            choices=["minimal", "full", "aws", "gcp", "multi-cloud"],
            default="minimal",
    ...
    def _validate_secrets_config(self, config_data: Dict[str, Any]) -> List[str]:
        ...
        valid_sections = ['aws', 'oci', 'gcp', 'cloudflare', 'ssh']

        if not any(section in config_data for section in valid_sections):
            errors.append("Secrets configuration should contain at least one platform section (aws, oci, gcp, cloudflare, ssh)")
